...
const scriptsTpl = `    <script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.2/gsap.min.js"></script>
...
for (const file of files) {
    ...
    // Inject Header
    // Need to set active class based on page
    let pageHeader = headerTpl.replace(new RegExp(`class="nav-link-${file.replace('.html','')}"`), `class="active"`);
    // If it's index, we don't have nav-link-index, we have nav-link-home
    if (file === 'index.html') {
    ...
    // Inject Scripts
    content = content.replace(/<!-- INCLUDE SCRIPTS -->[\s\S]*?<\/body>/g, scriptsTpl);
